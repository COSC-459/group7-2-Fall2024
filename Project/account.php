...
if($_SESSION['logged_in']){
  ...
  $stmt = $conn->prepare(query: "SELECT * FROM inquiry_units WHERE user_id=?");

  $stmt->bind_param('i', $user_id);

  $stmt->execute();

  $inq_u = $stmt->get_result();//[]
}
...
if($_SESSION['logged_in']){
  ...
  $stmt = $conn->prepare(query: "SELECT * FROM saved WHERE user_id=?");

  $stmt->bind_param('i', $user_id);

  $stmt->execute();

  $saved_u = $stmt->get_result();//[]
}
...
<head>
    ...
    <div class="container text-center mt-3 pt-5">
        <div class="mx-auto container">
            ...
            <div class="account-info">
                ...
                <!--<p><a href="inquiries.php" id="inq-btn">Your Inquiries</a></p>-->
                <!--<p><a href="save.php" id="saved-btn">Your Saves</a></p>-->
  ...
  <!--Inquiries-->
  <section class="inquiries container my-5 py-5">
  <table class="mt-5 pt-5">
      ...
      <?php while($row = $inq_u->fetch_assoc()){ ?>

        <tr>
            <td>
                <div class="unit-info">
                    <p class="mt-3"><?php echo $row['inquiry_id']; ?></p>
                </div>
            </td>
            <td>
            ...
            <td>
              <span><?php echo $row['unit_rent']; ?></span>
            </td>
        </tr>
  ...
  <table class="mt-5 pt-5">
      ...
      <?php while($row = $saved_u->fetch_assoc()){ ?>

        <tr>
            <td>
                <div class="unit-info">
                    <p class="mt-3"><?php echo $row['saved_id']; ?></p>
                </div>
            </td>
            <td>
              <span><?php echo $row['unit_name']; ?></span>
