...
if(isset($_POST['add_to_inquiries'])){
  //if added to nonempty save
  if(isset($_SESSION['inquiry'])){

    $units_array_id = array_column(array: $_SESSION['inquiry'], column_key: 'unit_id');

    //if unit already added or not
    if(!in_array(needle: $_POST['unit_id'], haystack: $units_array_id)){
      $unit_id = $_POST['unit_id'];
        ...
        $_SESSION['inquiry'][$unit_id] = $unit_array;
...
//remove from saves
  ...
  $unit_id = $_POST['unit_id'];
  unset($_SESSION['inquiry'][$unit_id]);
...
<head>
  ...
  <section class="inquiries container my-5 py-5">
    <div class="container mt-5">
        <h2 class="font-weight-bold">Your Inquiries</h2>
        <hr>
    </div>
    ...
    <table class="mt-5 pt-5">
        ...
        <?php foreach($_SESSION['inquiry'] as $key => $value){ ?>
    ...
    <div class="inquiry-quantity">
        <table>
            <tr>
                <form method="POST" action="inquiry_form.php">
                  <input type="hidden" name="unit_id" value="<?php echo $value['unit_id']; ?>"/> 
                  <input type="submit" class="quan-btn" value="Make an Inquiry" name="fill_inquiry"/> 
                </form>
            </tr>
        </table>
    </div>
